...
import osload_dotenv()BOT_TOKEN = os.getenv('BOT_TOKEN')
CHAT_ID = os.getenv('CHAT_ID')exchange = ccxt.bybit({'enableRateLimit': True, 'options': {'defaultType': 'linear'}})telegram_bot = Bot(token=BOT_TOKEN)signal_cache = {}  # Duplicate önlemedef calculate_rsi(closes, period=14):
    ...
    rsi[:period] = 100. - 100. / (1. + rs)for i in range(period, len(closes)):
    delta = deltas[i-1]
    if delta > 0:
        upval = delta
        downval = 0.
...
return rsidef calculate_rsi_ema(rsi, ema_length=14):
    ...
    return emadef find_local_extrema(arr, order=2):
    highs = []
    lows = []
    for i in range(order, len(arr) - order):
        ...
        if arr[i] == min(arr[i-order:i+order+1]):
            lows.append(i)
    return np.array(highs), np.array(lows)async def check_divergence(symbol, timeframe):
    try:
        ohlcv = exchange.fetch_ohlcv(symbol, timeframe, limit=50)  # Son 50 mum (35+ buffer)
        closes = np.array([x[4] for x in ohlcv])
        rsi = calculate_rsi(closes, 14)
        rsi_ema = calculate_rsi_ema(rsi, 14)
        rsi_ema2 = np.roll(rsi_ema, 2)  # EMA[2]    # EMA crossover and gray zone
    ...
    print(f"Hata ({symbol} {timeframe}): {str(e)}")async def main():
